...
function Container({ children }: LegalTextProps) {
  return (
    <main className="mx-auto flex w-full max-w-[1000px] flex-col px-6 py-16 text-on-background md:px-8 md:py-20">
      {children}
    </main>
  )
}

function Title({ children }: LegalTextProps) {
  return <p className="mx-auto my-1 text-center">{children}</p>
}

function Body({ children }: LegalTextProps) {
  return <div className="my-3 w-full text-left">{children}</div>
}

function Section({ children }: LegalTextProps) {
  return <div className="mt-8 mb-4 w-full font-bold">{children}</div>
}

function Subsection({ children }: LegalTextProps) {
  return <div className="my-3 mr-auto ml-4 w-full">{children}</div>
}

function InnerSubsection({ children }: LegalTextProps) {
  return <div className="my-3 mr-auto ml-8 w-full">{children}</div>
}

const TEXT = {
  Body({ children }: LegalTextProps) {
    return <span className="text-base leading-relaxed text-on-background">{children}</span>
  },
  BodyBold({ children }: LegalTextProps) {
    return (
      <strong className="text-base font-bold leading-relaxed text-on-background">
        {children}
      </strong>
    )
  },
